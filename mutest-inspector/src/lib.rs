...
use crate::ctxt::{TargetCtxt, TargetSpec, WorkspaceCtxt};
...
fn register_nested_target_metadata_dirs(targets: &mut Vec<TargetMetadataDir>, package: &str, into_target_spec: fn(String) -> TargetSpec, json_nested_targets_dir_path: &Path) {
    let read_json_nested_targets_dir = match fs::read_dir(&json_nested_targets_dir_path) {
        Ok(v) => v,
        Err(err) if let io::ErrorKind::NotFound = err.kind() => { return; }
        Err(err) => panic!("cannot read json package nested targets directory `{}`: {err}", json_nested_targets_dir_path.display()),
    };

    for entry in read_json_nested_targets_dir {
        let entry = entry.expect(&format!("cannot read entry in json package nested targets directory `{}`", json_nested_targets_dir_path.display()));
        let json_nested_target_dir_path = entry.path();
        let target_name = json_nested_target_dir_path.file_name().expect("invalid json nested target directory path")
            .to_str().expect("invalid target directory name")
            .to_owned();

        register_target_metadata_dir_if_exists(targets, package, into_target_spec(target_name), json_nested_target_dir_path);
    }
}
...
fn discover_target_metadata_dirs_in_json_root_dir(metadata_root_dir_path: &Path) -> Vec<TargetMetadataDir> {
    ...
    for entry in fs::read_dir(metadata_root_dir_path).expect(&format!("cannot read metadata root directory `{}`", metadata_root_dir_path.display())) {
        ...
        let package_name = json_package_dir_path.file_name().expect("invalid json package directory path")
            .to_str().expect("invalid package directory name")
            .to_owned();

        register_target_metadata_dir_if_exists(&mut target_metadata_dirs, &package_name, TargetSpec::Lib, json_package_dir_path.join("lib"));
        register_target_metadata_dir_if_exists(&mut target_metadata_dirs, &package_name, TargetSpec::MainBin, json_package_dir_path.join("bin"));
        register_nested_target_metadata_dirs(&mut target_metadata_dirs, &package_name, TargetSpec::Bin, &json_package_dir_path.join("bins"));
        register_nested_target_metadata_dirs(&mut target_metadata_dirs, &package_name, TargetSpec::Example, &json_package_dir_path.join("examples"));
        register_nested_target_metadata_dirs(&mut target_metadata_dirs, &package_name, TargetSpec::Test, &json_package_dir_path.join("tests"));
    }
...
struct TargetMetadata {
    ...
    evaluation_metadata: Option<mutest_json::evaluation::EvaluationInfo>,
    unique_source_file_paths: HashSet<PathBuf>,
}
...
fn load_target_metadata(target_metadata_dir: TargetMetadataDir) -> TargetMetadata {
    ...
    let evaluation_metadata_file_str = match fs::read_to_string(&evaluation_metadata_file_path) {
        ...
        Err(err) => panic!("cannot read evaluation metadata file `{}`: {err}", evaluation_metadata_file_path.display()),
    };
    ...
    // NOTE: First collect path references into a unique set to avoid having to allocate every path for comparisons.
    let unique_source_file_path_refs = call_graph_metadata.definitions.iter()
        .filter_map(|def| def.span.as_ref().map(|span| -> &Path { &span.path }))
        .collect::<HashSet<_>>();
    let unique_source_file_paths = unique_source_file_path_refs.into_iter().map(|p| p.to_owned()).collect::<HashSet<_>>();

    TargetMetadata {
        package: target_metadata_dir.package,
        target: target_metadata_dir.target,
        call_graph_metadata,
        mutations_metadata,
        evaluation_metadata,
...
pub async fn run(config: Config) {
    ...
    let package_count = target_metadata_dirs.iter().map(|target_metadata_dir| &target_metadata_dir.package).collect::<HashSet<_>>().len();
    println!("found {} packages with {} targets", package_count, target_metadata_dirs.len());

    let mut loaded_target_metadatas = Vec::new();
    let mut source_file_paths = HashSet::new();

    for target_metadata_dir in target_metadata_dirs {
        let target_metadata = load_target_metadata(target_metadata_dir);
        source_file_paths.extend(target_metadata.unique_source_file_paths.clone());
        loaded_target_metadatas.push(target_metadata);
    }

    let mut wcx = WorkspaceCtxt::empty();

    println!("processing {} source files", source_file_paths.len());
    ...
    let mut source_file_paths = source_file_paths.into_iter().map(|p| p.to_owned()).collect::<Vec<_>>();
    ...
    for target_metadata in loaded_target_metadatas {
        let mut tcx = TargetCtxt::new(&wcx, target_metadata.unique_source_file_paths, &target_metadata.call_graph_metadata, &target_metadata.mutations_metadata);

        let mut mutations_per_file = HashMap::<PathBuf, Vec<MutationId>>::new();
        for mutation in &target_metadata.mutations_metadata.mutations {
        ...
        println!("processing {} mutations", target_metadata.mutations_metadata.mutations.len());

        let mut mutation_htmls = IdxVec::<MutationId, _>::with_capacity(target_metadata.mutations_metadata.mutations.len());
        for mutation in &target_metadata.mutations_metadata.mutations {
        ...
        tcx.register_loaded_mutations(mutation_htmls);

        if let Some(evaluation_metadata) = &target_metadata.evaluation_metadata {
            let mut evaluation_info = EvaluationInfo {
                mutation_detections: IdxVec::with_capacity(target_metadata.mutations_metadata.mutations.len()),
                test_runs: HashMap::with_capacity(tcx.tests_count()),
            };
            ...
            for (runtime_test_id, mutation_detections) in mutation_run.mutation_detection_matrix.test_detections.iter_enumerated() {
                ...
                let reachable_targets = target_metadata.mutations_metadata.targets.iter()
                ...
                for (mutation_id, mutation_detection) in mutation_detections.0.iter_enumerated() {
                    let mutation = &target_metadata.mutations_metadata.mutations[mutation_id];
            ...
            tcx.update_evaluation_info(Some(evaluation_info));
        }

        wcx.register_target(target_metadata.package, target_metadata.target, tcx);
    }
