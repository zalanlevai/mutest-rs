pub fn command() -> clap::Command {
    let cmd = clap::command!("cargo mutest")
        ...
        .arg(clap::arg!(--"mutant-batch-algorithm" [MUTANT_BATCH_ALGORITHM] "Algorithm to use to batch mutations into mutants.").value_parser(["greedy", #[cfg(feature = "random")] "random", "none"]).default_value("none").display_order(100))
        ...
        .arg(clap::arg!(--"mutant-batch-greedy-ordering-heuristic" [MUTANT_BATCH_GREEDY_ORDERING_HEURISTIC] "Ordering heuristic to use for `greedy` mutation batching algorithm.").value_parser(["conflicts", "reverse-conflicts", #[cfg(feature = "random")] "random", "none"]).default_value("reverse-conflicts").display_order(100))
        .arg(clap::arg!(--"mutant-batch-greedy-epsilon" [MUTANT_BATCH_GREEDY_EPSILON] "Optional epsilon parameter for `greedy` mutation batching algorithm, used to control the probability of random mutation assignment.").default_value("0").value_parser(clap::value_parser!(f64)).display_order(100))
    ...
    #[cfg(feature = "random")]
    let cmd = cmd
        .arg(clap::arg!(--"mutant-batch-seed" [MUTANT_BATCH_SEED] "Random seed to use for randomness during mutation batching.").display_order(100))
        .arg(clap::arg!(--"mutant-batch-random-attempts" [MUTANT_BATCH_RANDOM_ATTEMPTS] "Number of attempts made to place a mutation into a random mutant for random choice during mutation batching.").default_value("1").value_parser(clap::value_parser!(usize)).display_order(100));
