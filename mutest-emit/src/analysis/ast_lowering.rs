    ...
    pub fn from_resolver(resolver: &ResolverAstLowering) -> Self {
        let node_id_to_def_id = resolver.owners.items()
            .flat_map(|(_, owner_resolver_data)| {
                UnordItems::new(iter::once((owner_resolver_data.id, owner_resolver_data.def_id)))
                    .chain(owner_resolver_data.node_id_to_def_id.items().map(|(&node_id, &def_id)| (node_id, def_id)))
            })
            .collect();
...
pub mod visit {
    ...
    pub fn walk_expr<'ast, 'hir, T: AstHirVisitor<'ast, 'hir>>(visitor: &mut T, expr_ast: &'ast ast::Expr, expr_hir: &'hir hir::Expr<'hir>) {
        match (&expr_ast.kind, &expr_hir.kind) {
            ...
            (ast::ExprKind::ForLoop(for_loop_ast), hir::ExprKind::Match(outer_match_expr_hir, [outer_match_arm_hir], hir::MatchSource::ForLoopDesugar)) => {
                if let hir::ExprKind::Loop(inner_loop_block_hir, _inner_loop_label_hir, hir::LoopSource::ForLoop, _) = outer_match_arm_hir.body.kind {
                        ...
                        && let hir::ExprKind::Match(_, [_, inner_loop_match_some_arm_hir], hir::MatchSource::ForLoopDesugar) = inner_loop_block_expr_hir.kind
                    {
                        if let hir::PatKind::Struct(_, [inner_loop_match_some_arm_pat_field_hir], _) = inner_loop_match_some_arm_hir.pat.kind {
                            visit_matching_pat(visitor, &for_loop_ast.pat, inner_loop_match_some_arm_pat_field_hir.pat);
                        }
                        match for_loop_ast.kind {
                            ast::ForLoopKind::For => {
                                if let hir::ExprKind::Call(_, [outer_match_expr_iter_hir]) = outer_match_expr_hir.kind {
                                    visit_matching_expr(visitor, &for_loop_ast.iter, outer_match_expr_iter_hir);
                                }
                            }
                        ...
                        visit_block_expr(visitor, &for_loop_ast.body, inner_loop_match_some_arm_hir.body);
                    }
                }
            }
        ...
        fn visit<'ast, 'hir, T: AstHirVisitor<'ast, 'hir>>(&'ast self, visitor: &mut T, node_hir: hir::Node<'hir>) -> bool {
            ...
            match &self.kind {
                ...
                ast::ItemKind::Trait(trait_ast) => {
                    let hir::Node::Item(item_hir) = node_hir else { panic!("mismatched HIR node") };
                    let hir::ItemKind::Trait { impl_restriction: _, constness: _, is_auto: _, safety: _, ident: _, generics: generics_hir, bounds: generic_bounds_hir, items: _ } = &item_hir.kind else { panic!("mismatched HIR node") };
        ...
        .map(|(parent_node, current_node)| {
            ...
            match parent_node {
                ...
                hir::Node::Item(parent_item) => match &parent_item.kind {
                    ...
                    hir::ItemKind::Trait { impl_restriction: _, constness: _, is_auto: _, safety: _, ident: _, generics: _, bounds: _, items: parent_items } => {
