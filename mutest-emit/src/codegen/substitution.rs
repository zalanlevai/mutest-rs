...
pub fn expand_subst_match_expr(sp: Span, subst_loc: SubstLoc, original: Option<P<ast::Expr>>, substs: &Vec<(MutId, &Subst)>) -> P<ast::Expr> {
    ...
    // m.substitutions.$subst_loc_id
    let mutant_lookup_expr = ast::mk::expr_field_deep(sp,
    ...
    let mutant_lookup_ref_expr = ast::mk::expr_method_call(sp,
        mutant_lookup_expr,
        ast::mk::path_segment(sp, Ident::new(Symbol::intern("as_ref"), sp), vec![]),
        vec![],
    );
    ...
    let subst_lookup_expr = ast::mk::expr_method_call(sp,
        ...
        ast::mk::path_segment(sp, Ident::new(Symbol::intern("and_then"), sp), vec![]),
        vec![ast::mk::expr_closure(sp, vec![Ident::new(*sym::mutant, sp)], mutant_lookup_ref_expr)],
    );
